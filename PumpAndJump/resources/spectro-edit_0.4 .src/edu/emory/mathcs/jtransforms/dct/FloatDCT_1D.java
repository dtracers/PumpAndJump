...
public class FloatDCT_1D
{
  ...
  public FloatDCT_1D(int paramInt)
  {
    ...
    this.n = paramInt;
    this.ip = new int[(int)Math.ceil(2 + (1 << (int)(Math.log(paramInt / 2 + 0.5D) / Math.log(2.0D)) / 2))];
    this.w = new float[paramInt * 5 / 4];
  }

  public FloatDCT_1D(int paramInt, int[] paramArrayOfInt, float[] paramArrayOfFloat)
  {
    if (!ConcurrencyUtils.isPowerOf2(paramInt))
      throw new IllegalArgumentException("Dimension must be power of two");
    if (paramInt < 1)
  ...
  public void forward(float[] paramArrayOfFloat, int paramInt, boolean paramBoolean)
  {
    if (this.n == 1)
      return;
    int j = this.ip[0];
    ...
    {
      paramArrayOfFloat[(paramInt + i + 1)] = (paramArrayOfFloat[(paramInt + i)] - paramArrayOfFloat[(paramInt + i - 1)]);
      paramArrayOfFloat[(paramInt + i)] += paramArrayOfFloat[(paramInt + i - 1)];
    }
    paramArrayOfFloat[(paramInt + 1)] = (paramArrayOfFloat[paramInt] - f);
    paramArrayOfFloat[paramInt] += f;
    if (this.n > 4)
    {
      rftbsub(this.n, paramArrayOfFloat, paramInt, k, this.w, j);
      cftbsub(this.n, paramArrayOfFloat, paramInt, this.ip, j, this.w);
    }
    else if (this.n == 4)
    {
      cftbsub(this.n, paramArrayOfFloat, paramInt, this.ip, j, this.w);
    }
    dctsub(this.n, paramArrayOfFloat, paramInt, k, this.w, j);
    if (paramBoolean)
    {
      scale(paramArrayOfFloat, paramInt);
      paramArrayOfFloat[paramInt] /= (float)Math.sqrt(2.0D);
    }
  }
  ...
  public void inverse(float[] paramArrayOfFloat, int paramInt, boolean paramBoolean)
  {
    ...
    if (this.n > 4)
    {
      cftfsub(this.n, paramArrayOfFloat, paramInt, this.ip, j, this.w);
      rftfsub(this.n, paramArrayOfFloat, paramInt, k, this.w, j);
    }
    else if (this.n == 4)
    {
      cftfsub(this.n, paramArrayOfFloat, paramInt, this.ip, j, this.w);
    ...
    paramArrayOfFloat[(paramInt + this.n - 1)] = f;
  }
      ...
      if (i != j)
        arrayOfFuture[(i1++)] = ConcurrencyUtils.threadPool.submit(new Runnable()
        {
          public void run()
          {
            ...
            {
              m >>= 2;
              FloatDCT_1D.this.cftmdl1(m, paramArrayOfFloat1, n - m, paramArrayOfFloat2, paramInt3 - (m >> 1));
            }
            FloatDCT_1D.this.cftleaf(m, 1, paramArrayOfFloat1, n - m, paramInt3, paramArrayOfFloat2);
            int k = 0;
            int i1 = i3 - m;
            int j = i2 - m;
            while (j > 0)
            {
              k++;
              int i = FloatDCT_1D.this.cfttree(m, j, k, paramArrayOfFloat1, i3, paramInt3, paramArrayOfFloat2);
              FloatDCT_1D.this.cftleaf(m, i, paramArrayOfFloat1, i1 + j, paramInt3, paramArrayOfFloat2);
              j -= m;
            }
          }
        });
        ...
        arrayOfFuture[(i1++)] = ConcurrencyUtils.threadPool.submit(new Runnable()
        {
          public void run()
          {
            ...
            while (m > 512)
            {
              m >>= 2;
              k <<= 2;
              FloatDCT_1D.this.cftmdl2(m, paramArrayOfFloat1, n - m, paramArrayOfFloat2, paramInt3 - m);
            }
            FloatDCT_1D.this.cftleaf(m, 0, paramArrayOfFloat1, n - m, paramInt3, paramArrayOfFloat2);
            k >>= 1;
            int i1 = i3 - m;
            int j = i2 - m;
            while (j > 0)
  ...
  private void rftfsub(int paramInt1, float[] paramArrayOfFloat1, int paramInt2, int paramInt3, float[] paramArrayOfFloat2, int paramInt4)
  {
    ...
    for (int i = 2; i < i1; i += 2)
    {
      int j = paramInt1 - i;
      k += m;
      float f1 = 0.5F - paramArrayOfFloat2[(paramInt4 + paramInt3 - k)];
  ...
  private void dctsub(int paramInt1, float[] paramArrayOfFloat1, int paramInt2, int paramInt3, float[] paramArrayOfFloat2, int paramInt4)
  {
    int i1 = paramInt1 >> 1;
    ...
    for (int i = 1; i < i1; i++)
    {
      ...
      int i3 = paramInt2 + i;
      int i4 = paramInt2 + j;
      float f1 = paramArrayOfFloat2[i2] - paramArrayOfFloat2[(paramInt4 + paramInt3 - k)];
      float f2 = paramArrayOfFloat2[i2] + paramArrayOfFloat2[(paramInt4 + paramInt3 - k)];
      float f3 = f2 * paramArrayOfFloat1[i3] - f1 * paramArrayOfFloat1[i4];
      paramArrayOfFloat1[i3] = (f1 * paramArrayOfFloat1[i3] + f2 * paramArrayOfFloat1[i4]);
      paramArrayOfFloat1[i4] = f3;
    }
    paramArrayOfFloat1[(paramInt2 + i1)] *= paramArrayOfFloat2[paramInt4];
  }

  private void scale(final float[] paramArrayOfFloat, final int paramInt)
  {
    int i = ConcurrencyUtils.getNumberOfProcessors();
    final float f = (float)Math.sqrt(2.0D / this.n);
    InterruptedException localInterruptedException1;
    if ((i > 1) && (this.n > ConcurrencyUtils.getThreadsBeginN_1D_FFT_2Threads()))
    {
      localInterruptedException1 = this.n / i;
      Future[] arrayOfFuture = new Future[i];
      for (int j = 0; j < i; j++)
      {
        final int k = j * localInterruptedException1;
        final int m;
        if (j == i - 1)
          m = this.n;
        else
          m = k + localInterruptedException1;
    ...
    {
      localInterruptedException1 = paramInt;
      InterruptedException localInterruptedException2 = paramInt + this.n;
      for (localInterruptedException3 = localInterruptedException1; localInterruptedException3 < localInterruptedException2; localInterruptedException3 += 2)
      {
        paramArrayOfFloat[localInterruptedException3] *= f;
        paramArrayOfFloat[(localInterruptedException3 + 1)] *= f;
      }
    }
  }
}
 ...
 * Qualified Name:     edu.emory.mathcs.jtransforms.dct.FloatDCT_1D
